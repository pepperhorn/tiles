...
export type PlayEvent = { midi: number | null; dur: number };
...
export function usePiano() {
  ...
  const playSequence = useCallback(async (events: PlayEvent[]) => {
    const { ctx, player } = await ensure();
    if (ctx.state === 'suspended') await ctx.resume();
    player.stop();
  ...
  const stop = useCallback(() => { engineRef.current?.player.stop(); }, []);
