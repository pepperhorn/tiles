...
test('the header tools toggle shows and hides the designer panel', async () => {
  render(<App />);
  // Designer is the default mode; the toggle lives in App's header (synchronous),
  // but DesignerMode now loads lazily, so wait for its panel to mount.
  const toggle = screen.getByRole('button', { name: 'Hide tools' });
  await waitFor(() => expect(document.querySelector('.designer-panel.flex')).toBeTruthy());
